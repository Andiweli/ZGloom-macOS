...
void GloomMap::ExecuteEvent(uint32_t e, bool& gotele, Teleport& teleout, bool allowTeleport)
{
	...
	//teleports
	// The original Amiga level-exit trigger still executes its event script,
	// but when finished2 is active it skips normal teleport animation.
	// allowTeleport lets GameLogic mirror that behaviour.
	if (!allowTeleport)
	{
		return;
	}
